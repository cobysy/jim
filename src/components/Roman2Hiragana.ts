...
class _Roman2Hiragana {
    private _get_hiragana(txt: string): Match {
        const ret = new Match();
        // tslint:disable-next-line:prefer-for-of
        for (let idx = 0; idx < _Roman2Hiragana.chr_convtbl.roma2hiragana.length; idx++) {
            const item = _Roman2Hiragana.chr_convtbl.roma2hiragana[idx];
            const match = txt.match(item.replace);
            if (match) {
                ret.match = true;
                ret.txt = txt.replace(item.replace, item.with);
                ret.val = item.hiragana;
                break;
            }
        }
        return ret;
    ...
    private _get_katakana(txt: string): Match {
        const chr = txt.substr(0, 1);
        const t = _Roman2Hiragana.chr_convtbl.hiragana2katakana[chr];
        const ret: Match = { match: false, txt: txt.substring(1) };
        if (t) {
            ret.match = true;
            ret.val = t.zenkaku;
        }
    ...
    private _conv(in_txt: string, get_func: (string) => Match) {
        const ret = { text: "", complete: false };
        let txt = in_txt;
        while (txt) {
            // tslint:disable-next-line:variable-name
            const conved_item = get_func(txt);
            if (conved_item.match) {
                ret.text = ret.text + conved_item.val;
                ret.complete = true;
                txt = conved_item.txt;
            } else {
                const chr = txt.substr(0, 1);
                const dbl = _Roman2Hiragana.chr_convtbl.hankaku2zenkaku[chr];
                if (dbl) {
    ...
    public conv(in_txt: string) {
        return this._conv(in_txt, this._get_hiragana);
    }

    // tslint:disable-next-line:member-ordering variable-name
    public katakana(in_txt: string) {
        return this._conv(in_txt, this._get_katakana);
    }

    // tslint:disable-next-line:member-ordering variable-name
    private static chr_convtbl = {
        hankaku2zenkaku: {
            " ": "　",
            "!": "！",
            "\"": "”",
...
export const Roman2Hiragana = new _Roman2Hiragana();
